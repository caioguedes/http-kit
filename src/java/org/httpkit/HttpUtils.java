...
public class HttpUtils {
  ...
  private static int initMappedThreshold() {
    return 1024 * 1024 * Integer.getInteger("org.http-kit.memmap-file-threshold", 20);
  }
  ...
  public static final Set<String> NON_TEXT_CONTENT_TYPES =
  ...
  public static int getPort(URI uri) {
    ...
    if (port == -1) {
      if ("https".equals(uri.getScheme())) port = 443;
      else port = 80;
    }
    return port;
  }
  ...
  public static void printError(String msg, Throwable t) {
    String error =
        String.format("%s [%s] ERROR - %s", new Date(), Thread.currentThread().getName(), msg);
  ...
  static final Pattern ENCODING =
      Pattern.compile("encoding=('|\")([\\w|-]+)('|\")", Pattern.CASE_INSENSITIVE);
  ...
  public static ByteBuffer[] HttpEncode(
      int status, HeaderMap headers, Object body, String serverHeader) {
  ...
  public static ByteBuffer[] HttpEncode(
    ...
    try {
      ...
      if (!CHUNKED.equals(headers.get("Transfer-Encoding"))) {
        if (legacyContentLength) {
          ...
          // New behavior: if present, respect Content-Length provided by the user from the handler
          // by not touching that header
    ...
    if (bodyBuffer != null) return new ByteBuffer[] {headBuffer, bodyBuffer};
    else return new ByteBuffer[] {headBuffer};
  }
