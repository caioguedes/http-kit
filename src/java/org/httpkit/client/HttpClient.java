...
public class HttpClient implements Runnable {
  ...
  public static interface AddressFinder {
    public static final AddressFinder DEFAULT =
        new AddressFinder() {
  ...
  public static interface ChannelFactory {
    public static final ChannelFactory DEFAULT =
        new ChannelFactory() {
  ...
  public static interface SSLEngineURIConfigurer {
    public static final SSLEngineURIConfigurer NOP =
        new SSLEngineURIConfigurer() {
  ...
  public HttpClient(
      ...
      SSLEngineURIConfigurer sslEngineUriConfigurer,
  ...
  /**
   * http-kit think all connections are keep-alived (since some say it is, but actually is not).
   * but, some are not, http-kit pick them out after the fact
   *
   * <ol>
   *   <li>The connection is reused
   *   <li>No data received
   * </ol>
   */
  ...
  private void doRead(SelectionKey key, long now) {
    ...
    try {
      ...
      // java.security.InvalidAlgorithmParameterException: Prime size must be multiple of 64, and
      // can only range from 512 to 1024 (inclusive)
      ...
      try {
        ...
        if (req.decoder.decode(buffer) == ALL_READ) {
          ...
          if (req.cfg.keepAlive > 0) {
            // Ensure that the key is added to keepalives exactly once on a state transition. There
            // could be cases where decoder reaches
            // ALL_READ state multiple times.
            if (oldState != ALL_READ) {
              keepalives.offer(
                  new PersistentConn(now + req.cfg.keepAlive, req.addr, req.host, key));
            }
          } else {
            closeQuietly(key);
          }
  ...
  private ByteBuffer[] encode(HttpMethod method, HeaderMap headers, Object body, String path)
      throws IOException {
    ...
    while (running) {
      try {
        ...
        processPending();
          ...
          Throwable e) { // catch any exception (including OOM), print it: do not exits the loop
