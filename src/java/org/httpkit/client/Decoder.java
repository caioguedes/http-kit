...
public class Decoder {
  ...
  public State decode(ByteBuffer buffer)
      throws LineTooLargeException, ProtocolException, AbortException {
    String line;
  ...
  private void readHeaders(ByteBuffer buffer)
      throws LineTooLargeException, AbortException, ProtocolException {
    String line = lineReader.readLine(buffer);
    while (line != null && !line.isEmpty()) {
      HttpUtils.splitAndAddHeader(line, headers);
      line = lineReader.readLine(buffer);
    }
    if (line == null) return; // data is not received enough. for next run
    listener.onHeadersReceived(headers);
    ...
    if (CHUNKED.equals(te)) {
      ...
      if (headers.containsKey(TRAILER)) chunkTrailerExpected = true;
    } else {
