      ...
      switch (state) {
        ...
        case READ_8_LENGTH:
          if (isAvailable(buffer, 8)) {
            ...
            if (length < 65536) {
              throw new ProtocolException(
                  "invalid data frame length. most significant bit is not zero or length fits in"
                      + " unsigned short.");
            }
        ...
        case PAYLOAD:
          ...
          if (payloadRead == payloadLength) {
            if (finalFlag) {
              if (fragmentedOpCode > 0) opcode = fragmentedOpCode;
              fragmentedOpCode = -1;
              switch (opcode) {
                case OPCODE_TEXT:
              ...
              payloadRead = 0;
              if (opcode > 0) fragmentedOpCode = opcode;
              opcode = -1;
            }
          }
          break;
