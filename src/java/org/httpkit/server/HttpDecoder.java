...
public class HttpDecoder {
  ...
  /**
   * Pattern for matching numbers 0 to 255. We use this in the IPV4 address pattern to prevent
   * invalid sequences from be parsed by InetAddress.getByName and thus being treated as a name
  ...
  /**
   * Pattern for a port number. We are not as strict in our pattern matching as we are with ipv4
   * address and instead rely upon Integer.parseInt and a range check. Port 0 is invalid, so we
   * disallow that here.
   */
  private static final String PORT = "[1-9]\\d{0,4}";

  /**
   * The PROXY protocol header is quite strict in what it allows, specifying for example that only a
   * single space character (\x20) is allowed between components.
   */
  ...
  public HttpDecoder(
    ...
    this.proxyProtocolOption =
        (proxyProtocolOption == null) ? ProxyProtocolOption.DISABLED : proxyProtocolOption;

    this.state =
        (proxyProtocolOption == ProxyProtocolOption.DISABLED)
            ? State.READ_INITIAL
  ...
  private void createRequest(String sb) throws ProtocolException {
    ...
    if (cStart < cEnd) {
      try {
        ...
        request =
            new HttpRequest(method, sb.substring(bStart, bEnd), version, legacyUnsafeRemoteAddr);
      } catch (Exception e) {
  ...
  public HttpRequest decode(ByteBuffer buffer)
  ...
  private void readHeaders(ByteBuffer buffer)
      throws LineTooLargeException, RequestTooLargeException, ProtocolException {
  ...
  private void throwIfBodyIsTooLarge() throws RequestTooLargeException {
    if (readCount + readRemaining > maxBody) {
      throw new RequestTooLargeException(
          "request body " + (readCount + readRemaining) + "; max request body " + maxBody);
    }
  }
}
