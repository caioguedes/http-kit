...
public class AsyncChannel {
  ...
  private final AtomicReference<IFn> closeRingHandler = new AtomicReference<>(null);
  ...
  private final AtomicReference<IFn> pongHandler = new AtomicReference<>(null);
  ...
  public boolean serverClose(int status, String reason) {
    ...
    if (isWebSocket()) {
      server.tryWrite(
          key, WsEncode(OPCODE_CLOSE, ByteBuffer.allocate(2).putShort((short) status).array()));
    } else {
